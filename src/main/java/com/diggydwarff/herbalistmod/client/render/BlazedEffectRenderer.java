...
public class BlazedEffectRenderer {

    public static final int MAX_BLAZE_DISTANCE = 10;
    public static final ResourceLocation BLAZE_SHADER = new ResourceLocation(HerbalistMod.MODID,
            "shaders/post/blaze.json");
    public static final ResourceLocation BLAZE_TEXTURE = new ResourceLocation(HerbalistMod.MODID,
            "textures/overlay/blaze.png");
    ...
    public void renderOverlay(PoseStack pose) {
        RenderSystem.setShaderTexture(0, BlazedEffectRenderer.BLAZE_TEXTURE);
    ...
    public void onEffectTick(TickEvent.PlayerTickEvent event) {
        MobEffectInstance effect = event.player.getEffect(ModEffects.BLAZED.get());
        int duration = effect == null ? 0 : effect.getDuration();
        if (duration > 1) {
            if (!this.effectActiveLastTick) {
                this.effectActiveLastTick = true;

                Minecraft.getInstance().tell(() -> Minecraft.getInstance().gameRenderer.loadEffect(BLAZE_SHADER));
            }
        } else {
            if (this.effectActiveLastTick) {
