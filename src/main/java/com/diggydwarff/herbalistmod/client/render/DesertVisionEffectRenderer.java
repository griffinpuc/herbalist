...
public class DesertVisionEffectRenderer {
    ...
    public void onEffectTick(TickEvent.PlayerTickEvent event) {
        MobEffectInstance effect = event.player.getEffect(ModEffects.DESERT_VISION.get());
        int duration = effect == null ? 0 : effect.getDuration();
        if (duration > 1) {
            if (!this.effectActiveLastTick) {
                this.effectActiveLastTick = true;

                Minecraft.getInstance().tell(() -> Minecraft.getInstance().gameRenderer.loadEffect(DESERT_VISION_SHADER));
            }
        } else {
            if (this.effectActiveLastTick) {
