...
public class AmpedEffectRenderer {

    public static final int MAX_INTROSPECTION_DISTANCE = 10;
    public static final ResourceLocation AMPED_SHADER = new ResourceLocation(HerbalistMod.MODID,
            "shaders/post/amped.json");

    public static final ResourceLocation AMPED_TEXTURE = new ResourceLocation(HerbalistMod.MODID,
    ...
    public void renderOverlay(PoseStack pose) {
        RenderSystem.setShaderTexture(0, AmpedEffectRenderer.AMPED_TEXTURE);
    ...
    public void onEffectTick(TickEvent.PlayerTickEvent event) {
        MobEffectInstance effect = event.player.getEffect(ModEffects.AMPED.get());
        int duration = effect == null ? 0 : effect.getDuration();
        if (duration > 1) {
            if (!this.effectActiveLastTick) {
                this.effectActiveLastTick = true;

                Minecraft.getInstance().tell(() -> Minecraft.getInstance().gameRenderer.loadEffect(AMPED_SHADER));
            }
        } else {
            if (this.effectActiveLastTick) {
