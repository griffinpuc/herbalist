...
public class IntrospectiveIIEffectRenderer {
    ...
    public static final ResourceLocation INTROSPECTION_SHADER = new ResourceLocation(HerbalistMod.MODID,
            "shaders/post/introspectionii.json");
    ...
    public void renderOverlay(PoseStack pose) {
        RenderSystem.setShaderTexture(0, IntrospectiveIIEffectRenderer.INTROSPECTION_TEXTURE);
    ...
    public void onEffectTick(TickEvent.PlayerTickEvent event) {
        MobEffectInstance effect = event.player.getEffect(ModEffects.INTROSPECTIONII.get());
        int duration = effect == null ? 0 : effect.getDuration();
        if (duration > 1) {
            if (!this.effectActiveLastTick) {
                this.effectActiveLastTick = true;

                Minecraft.getInstance().tell(() -> Minecraft.getInstance().gameRenderer.loadEffect(INTROSPECTION_SHADER));
            }
        } else {
            if (this.effectActiveLastTick) {
